    ...
    <div className="space-y-6 pb-20">
      ...
      {!selectedPerson && (
        <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white p-4 rounded-lg border border-slate-100">
          <div className="flex gap-4 w-full sm:w-auto flex-1">
              <div className="relative w-full sm:w-72">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-lg focus:ring-1 focus:ring-slate-400 outline-none" 
                />
              </div>
              <div className="relative min-w-[160px]">
                <Icon name="sort" className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                <select
                    value={groupSort}
                    onChange={(e) => setGroupSort(e.target.value as SortOption)}
                    className="w-full pl-10 pr-8 py-2 border border-slate-200 rounded-lg focus:ring-1 focus:ring-slate-400 outline-none bg-white appearance-none cursor-pointer text-sm font-medium text-slate-600"
                >
          ...
          <div className="text-sm text-slate-500 whitespace-nowrap">Toplam <span className="font-bold text-slate-800">{summaries.length}</span> Personel</div>
        </div>
      )}

      {selectedPerson ? (
        <div className="bg-white rounded-lg border border-slate-100 overflow-hidden animate-fade-in">
          
          <div className="p-6 border-b border-slate-100 bg-slate-50">
            <div className="flex items-center justify-between mb-6">
               <button onClick={handleBackToList} className="flex items-center gap-2 text-slate-500 hover:text-slate-800 transition-colors">
                 <div className="p-2 bg-white rounded-full border border-slate-200 hover:bg-slate-100">
                   <Icon name="arrow_back" />
                 </div>
                 <span className="font-medium">Listeye Dön</span>
               </button>
               <div className="flex items-center gap-2">
                  <span className="px-3 py-1 bg-slate-100 text-slate-700 rounded-full text-xs font-bold uppercase tracking-wide">{selectedPerson.role || 'Personel'}</span>
                  {selectedPerson.bonus_percentage && selectedPerson.bonus_percentage > 0 ? (
                      <span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-xs font-bold uppercase tracking-wide">%{selectedPerson.bonus_percentage} Prim</span>
                  ) : null}
               </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
               <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-lg bg-primary-100 text-slate-600 flex items-center justify-center text-2xl font-bold border-4 border-white">
               ...
               <div className="flex flex-wrap gap-4 w-full lg:w-auto">
                  <button
                    ...
                    className={`flex-1 lg:flex-none p-4 bg-white rounded-lg border min-w-[160px] text-left transition-all ${
                        viewMode === 'current_account' ? 'ring-2 ring-purple-500 border-purple-500' : 'border-slate-200'
                    }`}
                  >
                     <div className="flex justify-between items-start mb-1">
                        <p className="text-xs text-slate-400 font-bold uppercase">Cari Bakiye</p>
                        <Icon name={viewMode === 'current_account' ? "check_circle" : "arrow_forward_ios"} className={`text-[10px] ${viewMode === 'current_account' ? "text-purple-500" : "text-slate-300"}`} />
                     </div>
                     <p className={`text-xl font-bold ${selectedPerson.currentAccountBalance < 0 ? 'text-slate-500' : 'text-slate-600'}`}>
                  ...
                  <button 
                    ...
                    className={`flex-1 lg:flex-none p-4 rounded-lg border min-w-[180px] flex items-center justify-center gap-2 transition-all group ${
                        viewMode === 'performance' ? 'bg-gray-800 text-white border-gray-800' : 'bg-slate-50 border-slate-200 text-slate-700 hover:bg-slate-100'
                    }`}
                  >
                  ...
                  {viewMode === 'current_account' && (
                    <button 
                       ...
                       className="px-4 rounded-lg bg-slate-800 text-white font-bold hover:bg-green-700 transition-colors flex items-center gap-2"
                    >
          ...
          <div className="p-6">
            {viewMode === 'performance' ? (
                <div className="animate-fade-in">
                    <div className="bg-slate-50 p-6 rounded-lg border border-amber-100 mb-6">
                    ...
                    <h3 className="font-bold text-slate-800 mb-4 text-lg">Dönemsel Performans & %40'lar Tablosu</h3>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-200">
                                    <th className="p-4 text-sm font-bold text-slate-600">Dönem</th>
                                    <th className="p-4 text-sm font-bold text-slate-600 text-center">İşlem Sayısı</th>
                                    <th className="p-4 text-sm font-bold text-slate-600 text-right">Dosya Geliri</th>
                                    <th className="p-4 text-sm font-bold text-slate-600 text-right">Dosya Gideri</th>
                                    <th className="p-4 text-sm font-bold text-slate-600 text-right">Net Ofis Kârı</th>
                                    <th className="p-4 text-sm font-bold text-slate-700 text-right bg-slate-50">%40'lar (%{selectedPerson.bonus_percentage})</th>
                                ...
                                {selectedPerson.quarterlyStats.map((q) => {
                                    ...
                                    <React.Fragment key={key}>
                                        <tr 
                                            ...
                                            className={`cursor-pointer transition-colors border-l-4 ${isExpanded ? 'bg-slate-50 border-blue-500' : 'hover:bg-slate-50 border-transparent'}`}
                                        >
                                            <td className="p-4 font-bold text-slate-800 flex items-center gap-2">
                                                <Icon name={isExpanded ? "expand_less" : "expand_more"} className="text-slate-400 text-lg" />
                                                {q.year} - {q.quarter}. Çeyrek
                                            </td>
                                            <td className="p-4 text-center text-slate-600">
                                                <span className="px-2 py-1 bg-white border border-slate-200 rounded-md text-xs font-bold">{q.transactionCount}</span>
                                            </td>
                                            ...
                                            <td className="p-4 text-right font-bold text-lg bg-opacity-50 bg-slate-50">
                                                <span className={q.shareAmount >= 0 ? 'text-slate-600' : 'text-slate-500'}>
                                            ...
                                            <td className="p-4 text-right">
                                                <button 
                                                    ...
                                                    className="flex items-center justify-center gap-1 px-3 py-1.5 bg-slate-800 text-white rounded-lg text-xs font-bold hover:bg-green-700 transition-colors"
                                                ...
                                                <td colSpan={7} className="p-4 animate-fade-in">
                                                    <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
                                                        <div className="px-4 py-2 bg-slate-100 border-b border-slate-200 flex justify-between items-center">
                                                            <span className="text-xs font-bold text-slate-500 uppercase">{key} Dönemi İşlem Dökümü</span>
                                                            <span className="text-xs text-slate-400">{q.transactionCount} kayıt listelendi</span>
                                                        </div>
                                                            ...
                                                            <tbody className="divide-y divide-gray-100">
                                                                {getTransactionsForQuarter(q.year, q.quarter).map(t => (
                                                                    <tr key={t.id} className="hover:bg-slate-50">
                                                                        <td className="p-3 text-xs font-mono text-slate-600">{formatDate(t.date)}</td>
                                                                        <td className="p-3 text-xs text-slate-800">{t.description}</td>
                                                                        <td className="p-3 text-xs text-slate-600">{t.client || t.counterparty || '-'}</td>
                                                                        <td className={`p-3 text-xs font-bold text-right ${t.type === TransactionType.INCOME ? 'text-slate-600' : 'text-slate-500'}`}>
                            ...
                            {selectedPerson.quarterlyStats.length > 0 && (
                                <tfoot className="bg-slate-50 border-t-2 border-slate-200">
                                    <tr>
                                        <td colSpan={5} className="p-4 text-right font-bold text-slate-600 uppercase">Genel Toplam %40'lar</td>
                                        <td className="p-4 text-right font-bold text-xl text-slate-700 bg-primary-100 border-l border-primary-200">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div>
                           <h3 className="font-bold text-purple-900 text-lg flex items-center gap-2">
                               <Icon name="account_balance_wallet" className="text-slate-600" />
                               Cari Hesap Hareketleri
                           </h3>
                           <p className="text-sm text-slate-500">Personelin avans, borç ve alacak işlemleri.</p>
                        </div>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                                ...
                                {financialTransactions.map((t) => {
                                    ...
                                    return (
                                        <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                                            <td className="py-4 pl-4 text-sm text-slate-600 font-mono whitespace-nowrap">{formatDate(t.date)}</td>
                                            <td className="py-4">
                                                <span className={`text-[10px] px-2 py-1 rounded font-bold border ${
                                                    t.type === TransactionType.DEBT ? 'bg-orange-50 text-slate-600 border-orange-100' :
                                                    t.type === TransactionType.RECEIVABLE ? 'bg-slate-50 text-slate-600 border-slate-200' :
                                                    'bg-purple-50 text-slate-600 border-purple-100'
                                                }`}>
                                                    {t.type}
                                                </span>
                                            </td>
                                            <td className="py-4 text-sm text-slate-800">{t.description}</td>
                                            <td className="py-4 text-sm text-slate-500">{t.category}</td>
                                            <td className={`py-4 pr-4 text-right font-bold text-sm ${
                                                isDebt ? 'text-slate-600' : 'text-slate-600'
                                            }`}>
                            ...
                            <tfoot className="bg-slate-50 border-t border-slate-200">
                                <tr>
                                    <td colSpan={4} className="p-4 text-right font-bold text-slate-600 uppercase">Güncel Bakiye</td>
                                    <td className={`p-4 text-right font-bold text-lg ${selectedPerson.currentAccountBalance < 0 ? 'text-slate-500' : 'text-slate-600'}`}>
                                        ₺{selectedPerson.currentAccountBalance.toLocaleString()}
                                    </td>
                                </tr>
                            </tfoot>
        ...
        <div className="space-y-6">
          {sortedRoleKeys.length > 0 ? sortedRoleKeys.map((role) => {
            ...
            return (
            <div key={role} className="bg-white rounded-lg border border-slate-100 overflow-hidden">
                <div 
                ...
                >
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-bold text-slate-800 text-lg">{role}</h3>
                            <p className="text-xs text-slate-500">{people.length} Personel</p>
                        </div>
                    </div>
                    <Icon name={expandedRoles[role] ? "expand_less" : "expand_more"} className="text-slate-400" />
                </div>
                                    ...
                                    >
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-bold text-gray-900 group-hover:text-slate-700 transition-colors">{person.full_name}</p>
                                                    <p className="text-xs text-slate-500">{person.role || 'Personel'}</p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-slate-500 hidden md:table-cell text-xs whitespace-nowrap">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <span className={`font-bold ${person.currentAccountBalance < 0 ? 'text-slate-500' : 'text-slate-600'}`}>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <Icon name="chevron_right" className="text-slate-300 group-hover:text-slate-600" />
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
             ...
             <div className="py-12 text-center">
              ...
              <p className="text-sm text-slate-400">Ayarlar sayfasından personel ekleyebilirsiniz.</p>
            </div>
          )}
        </div>
      )}
