    ...
    <div className="relative">
      ...
      <div className="relative">
        <input
          ...
          disabled={disabled}
          required={required}
          className={`w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none bg-white transition-shadow ${disabled ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : ''}`}
      ...
      {isOpen && filteredOptions.length > 0 && !disabled && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-100 rounded-xl shadow-xl max-h-48 overflow-y-auto">
          {filteredOptions.map((option, index) => (
            <div
              key={`${option}-${index}`}
              className="px-4 py-2.5 hover:bg-primary-50 hover:text-primary-700 cursor-pointer text-sm text-gray-700 transition-colors border-b border-gray-50 last:border-0"
...
export const TransactionModal: React.FC<TransactionModalProps> = ({ isOpen, onClose, onSave, initialData }) => {
  ...
  const TypeButton = ({ type, label, activeColor, icon }: { type: TransactionType, label: string, activeColor: string, icon: string }) => {
    ...
    return (
      <button
        ...
        className={`flex-1 py-2 px-1 rounded-lg text-xs font-medium transition-all flex flex-col items-center gap-1 border ${
          isActive
            ? `bg-white ${activeColor} border-current shadow-sm`
            : 'bg-transparent border-transparent text-gray-500 hover:bg-gray-100'
        }`}
      >
        <Icon name={icon} className={isActive ? '' : 'text-gray-400'} />
        {label}
      </button>
    );
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden transform transition-all max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 sticky top-0 z-10">
          <div className="flex flex-col">
              <h3 className="font-bold text-lg text-gray-800">
                {initialData ? 'İşlemi Düzenle' : 'Yeni İşlem Ekle'}
              </h3>
              <span className="text-xs font-mono text-gray-500 font-bold bg-gray-200 px-2 py-0.5 rounded mt-1 inline-block w-fit">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="bg-gray-50 p-1.5 rounded-xl flex gap-1">
            <TypeButton type={TransactionType.INCOME} label="Gelir" activeColor="text-emerald-600" icon="arrow_downward" />
            <TypeButton type={TransactionType.EXPENSE} label="Gider" activeColor="text-red-600" icon="arrow_upward" />
            <TypeButton type={TransactionType.RECEIVABLE} label="Alacak" activeColor="text-blue-600" icon="account_balance_wallet" />
            <TypeButton type={TransactionType.DEBT} label="Borç" activeColor="text-orange-600" icon="money_off" />
            <TypeButton type={TransactionType.CURRENT} label="Cari" activeColor="text-purple-600" icon="sync_alt" />
          </div>

          {formData.type === TransactionType.CURRENT && (
             <div className="bg-purple-50 p-4 rounded-xl border border-purple-100">
                 <label className="block text-xs font-bold text-purple-700 uppercase mb-2 text-center">Cari İşlem Yönü</label>
                 <div className="flex gap-4">
                     <button
                        type="button"
                        onClick={() => setCariDirection('PLUS')}
                        className={`flex-1 py-3 px-4 rounded-xl border text-sm font-bold flex items-center justify-center gap-2 transition-all ${
                            cariDirection === 'PLUS' 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div>
              ...
              <input
                ...
                min="0"
                step="0.01"
                className={`w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:border-transparent outline-none font-bold text-lg ${
                    formData.type === TransactionType.CURRENT && cariDirection === 'MINUS' 
                    ? 'text-orange-600 border-orange-200 focus:ring-orange-500' 
                    : 'text-gray-800 border-gray-200 focus:ring-primary-500'
                }`}
                placeholder="0.00"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {formData.type !== TransactionType.CURRENT && (
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Hesap / Kasa</label>
                <select
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none bg-white"
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Ödeme Yöntemi</label>
              {formData.type === TransactionType.CURRENT ? (
                 <input 
                    disabled 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Kimden (Personel)</label>
                <select
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none bg-white"
          ...
          <div className="flex gap-3 mt-8 pt-2">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-3 bg-gray-100 text-gray-700 rounded-xl font-bold hover:bg-gray-200 transition-colors">İptal</button>
            <button type="submit" disabled={loading} className="flex-1 px-4 py-3 bg-primary-700 text-white rounded-xl font-bold hover:bg-primary-800 transition-colors shadow-lg shadow-primary-200">
