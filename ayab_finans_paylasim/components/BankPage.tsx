...
const FlipCard: React.FC<{
  ...
  return (
    ...
    >
      <div
        className={`relative w-full h-full transition-all duration-500 [transform-style:preserve-3d] shadow-sm rounded-2xl ${
          isFlipped ? '[transform:rotateY(180deg)]' : ''
        }`}
      >
        <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] rounded-2xl bg-white overflow-hidden">
          {front}
        </div>
        <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] [transform:rotateY(180deg)] rounded-2xl bg-white overflow-hidden">
          {back}
        </div>
      </div>
    </div>
...
const AccountSection: React.FC<{
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-4">
      <div 
        onClick={onToggle}
        className="p-4 flex items-center justify-between cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors select-none"
      >
        ...
        <div className="flex items-center gap-4">
           ...
           <Icon name={isOpen ? "expand_less" : "expand_more"} className="text-gray-400" />
        </div>
      </div>
      ...
      {isOpen && (
        <div className="divide-y divide-gray-50">
          {accounts.map((account) => (
            <div key={account.id} className="p-4 hover:bg-gray-50 flex items-center justify-between group transition-colors">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-bold text-gray-800 text-sm">{account.bankName}</p>
                    <p className="text-xs text-gray-500">{account.type} {account.accountNumber !== '-' && `• ${account.accountNumber}`}</p>
                  </div>
               </div>
               
               <div className="flex items-center gap-4">
                 <div className="text-right">
                    <p className={`font-bold ${account.type.includes('Kredi') ? 'text-red-600' : 'text-emerald-600'}`}>
...
export const BankPage: React.FC<BankPageProps> = ({ accounts, transactions, onDataChange }) => {
  ...
  return (
    <div className="space-y-8 pb-20">
      {/* Info Banner */}
      <div className="flex items-center gap-2 text-sm text-gray-500 bg-blue-50 p-3 rounded-xl border border-blue-100">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <FlipCard
          front={
            <div className="h-full flex flex-col p-6 justify-between border border-gray-100 bg-white">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-bold text-gray-700">Likit Varlıklar</h3>
                  <p className="text-xs text-gray-400 mt-1">Nakit + Banka (TL Karşılığı)</p>
                </div>
                <div className="p-2 bg-emerald-50 rounded-lg">
                  <Icon name="savings" className="text-emerald-600 text-xl" />
                </div>
              </div>
              <div>
          ...
          back={
            <div className="h-full flex flex-col p-6 bg-gray-800 text-white justify-center space-y-4">
              <h4 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-2 border-b border-gray-700 pb-2">Varlık Dağılımı</h4>
               <div className="flex justify-between items-center">
                <span className="text-sm text-gray-300">TL Hesapları</span>
                <span className="font-bold">₺{tlAccounts.reduce((s,a)=>s+a.balance,0).toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-300">Döviz (TL Karş.)</span>
        ...
        <FlipCard
          front={
            <div className="h-full flex flex-col p-6 justify-between border border-gray-100 bg-white">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-bold text-gray-700">İşlem Dengesi</h3>
                  <p className="text-xs text-gray-400 mt-1">Sistemdeki Gelir - Gider</p>
                </div>
                <div className="p-2 bg-blue-50 rounded-lg">
                  <Icon name="sync_alt" className="text-blue-600 text-xl" />
                </div>
              </div>
              <div>
                <p className="text-3xl font-bold text-gray-900 tracking-tight">
          ...
          back={
             <div className="h-full flex flex-col p-6 bg-gray-50 border border-gray-200 justify-center space-y-3">
               <div className="flex justify-between text-sm">
                 <span className="text-emerald-600 flex items-center gap-1"><Icon name="arrow_downward" className="text-xs"/> Gelir</span>
                 <span className="font-bold text-gray-800">₺{totalIncome.toLocaleString()}</span>
               </div>
               <div className="flex justify-between text-sm">
                 <span className="text-red-600 flex items-center gap-1"><Icon name="arrow_upward" className="text-xs"/> Gider</span>
                 <span className="font-bold text-gray-800">₺{totalExpense.toLocaleString()}</span>
               </div>
               <div className="border-t border-dashed border-gray-200 my-1"></div>
                <div className="flex justify-between text-xs text-gray-500">
                 <span>Kredi Kartı Borcu:</span>
                 <span className="text-red-500 font-bold">₺{ccDebt.toLocaleString()}</span>
               </div>
            </div>
          }
        />
        ...
        <FlipCard
          front={
            <div className={`h-full flex flex-col p-6 justify-between border ${isDiscrepancyZero ? 'bg-emerald-50 border-emerald-100' : 'bg-orange-50 border-orange-100'}`}>
               <div className="flex justify-between items-start">
                <div>
                  <h3 className={`font-bold ${isDiscrepancyZero ? 'text-emerald-800' : 'text-orange-800'}`}>
                    {isDiscrepancyZero ? 'Mutabakat' : 'Kayıt Dışı Fark'}
                  </h3>
                  <p className={`text-xs mt-1 ${isDiscrepancyZero ? 'text-emerald-600' : 'text-orange-600'}`}>
                    Varlıklar vs İşlemler
                  </p>
                </div>
                <div className={`p-2 rounded-lg ${isDiscrepancyZero ? 'bg-emerald-100 text-emerald-600' : 'bg-orange-100 text-orange-600'}`}>
                  <Icon name={isDiscrepancyZero ? "check_circle" : "warning"} className="text-xl" />
                </div>
              </div>
              <div>
                <p className={`text-3xl font-bold tracking-tight ${isDiscrepancyZero ? 'text-emerald-700' : 'text-orange-700'}`}>
                  {isDiscrepancyZero ? 'Tamam' : `₺${Math.abs(discrepancy).toLocaleString('tr-TR', { minimumFractionDigits: 2 })}`}
                </p>
                <p className={`text-xs mt-2 font-medium ${isDiscrepancyZero ? 'text-emerald-600' : 'text-orange-600'}`}>
          ...
          back={
             <div className={`h-full flex flex-col p-6 justify-center items-center text-center ${isDiscrepancyZero ? 'bg-emerald-600 text-white' : 'bg-orange-600 text-white'}`}>
